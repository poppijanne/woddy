  ...
  res.on("finish", () => {
    log(
      `<- ${req.method} ${req.path} ${res.statusCode} (${
        new Date().getTime() - req.requestStart
      } ms, ${
  ...
  try {
    ...
    if (
      validatePassword(
        req.body.pass,
        users[0].passHash,
        users[0].salt + process.env.SECRET
      )
    ) {
...
app.post("/api/v1/user/register", async (req, res) => {
  ...
  try {
    ...
    if (users.length > 0) {
      res
        .status(400)
        .send("Sähköpostiosoite on jo käytössä toisella käyttäjällä.");
      return;
    }
      ...
      if (error) {
        ...
        if (
          validatePassword(
            req.body.pass,
            result.passHash,
            result.salt + process.env.SECRET
          )
        ) {
          const emailHash = generateHash(req.body.email, process.env.SECRET);
...
app.post("/api/v1/user/send-reset-password-link", async (req, res) => {
  ...
  try {
    ...
    const resetPasswordTokenHash = generateHash(
      resetPasswordToken,
...
app.post("/api/v1/contact", async (req, res) => {
  ...
  try {
    ...
    await transporter.sendMail({
      from:
        req.body.email ||
        `"${process.env.MAIL_SENDER_NAME}" <${process.env.MAIL_SENDER_ADDRESS}>`,
...
app.get("/api/v1/user/:userId/workouts", (req, res) => {
  ...
  pool.query(
    `SELECT workoutId, userId, createdTs, modifiedTs, summary FROM workouts WHERE userId = ? ORDER BY ?? ${sortOrder} LIMIT ? OFFSET ?`,
    [
      req.params.userId,
    ...
    function (error, results, fields) {
      if (error) {
        ...
        res.json(
          results.map(
            ({ workoutId, userId, createdTs, modifiedTs, summary }) => {
              return {
                workoutId,
                userId,
                createdTs,
...
app.put("/api/v1/workout", (req, res) => {
  ...
  pool.query(
    "UPDATE `workouts` SET ? WHERE workoutId = ? AND userId = ?",
...
app.delete("/api/v1/workout/:workoutId", (req, res) => {
  ...
  pool.query(
    "DELETE FROM `workouts` WHERE userId = ? AND workoutId = ?",
...
app.get("/api/v1/exercises", (req, res) => {
  res.json({
    exercises: [
      // Lämmittely
      {
        name: "Haaraperushyppy",
        muscles: ["Jalat"],
        tags: ["Lämmittely", "Kehonpaino", "Jalat"],
      },
      {
        name: "Hiihto",
        muscles: ["Jalat"],
      ...
      },
      {
        name: "Nyrkkeily",
        muscles: ["Jalat", "Kädet"],
        tags: ["Lämmittely", "Kehonpaino", "Kädet"],
      },
      {
        name: "Nopeat jalat",
        muscles: ["Jalat"],
      ...
      },
      {
        ...
        tags: ["Lämmittely", "Kehonpaino"],
      },
      // Jalat + pakarat
      {
        name: "Askelkyykky + nytkytys",
      ...
      {
        name: "Askelkyykky + kuula jalan alta",
        equipment: ["Kahvakuula"],
        muscles: ["Jalat"],
        tags: ["Jalat"],
      },
      {
        name: "Askelkyykky + pystypunnerrus",
        equipment: ["Kahvakuula"],
        muscles: ["Jalat", "Kädet"],
        tags: ["Jalat"],
      },
      {
        name: "Askelkyykky + työntö",
        equipment: ["Kahvakuula"],
        muscles: ["Jalat", "Kädet"],
        tags: ["Jalat", "Kädet"],
      },
      {
        name: "Sissikyykky polvillaan",
        equipment: ["Matto"],
        muscles: ["Jalat", "Vatsat"],
        tags: ["Kehonpaino", "Jalat"],
      },
      {
        name: "Kahdeksikko",
        equipment: ["Kahvakuula"],
      ...
      {
        name: "Thruster yhdellä kädellä",
        equipment: ["Kahvakuula"],
        muscles: ["Jalat", "Pakarat"],
        tags: ["Jalat"],
        sideSwitch: true,
      },
      {
        name: "Kyykky",
        equipment: ["Kahvakuula"],
        muscles: ["Jalat", "Takamus"],
        tags: ["Jalat"],
      },
      {
        name: "Kyykky + pystypunnerrus",
        equipment: ["Kahvakuula"],
        muscles: ["Jalat", "Takamus", "Kädet", "Hartiat"],
        tags: ["Jalat", "Kädet"],
      },
      {
        name: "Sumokyykky",
      ...
      {
        name: "Sumokyykky + pystysoutu",
        equipment: ["Kahvakuula"],
        muscles: ["Kädet", "Jalat", "Pakarat"],
      },
      {
        name: "Pissaava koira",
        equipment: ["Matto"],
      ...
      {
        name: "Hauiskääntö",
        muscles: ["Kädet", "Hauikset"],
        equipment: ["Kahvakuula"],
        tags: ["Kädet"],
      },
      {
        name: "Halo",
        muscles: ["Kädet", "Hartiat"],
        equipment: ["Kahvakuula"],
        tags: ["Kädet"],
      },
      {
        name: "Halo + hauiskääntö",
        equipment: ["Kahvakuula"],
        muscles: ["Kädet", "Hartiat", "Hauikset"],
        tags: ["Kädet"],
      },
      {
        name: "Ojentajat",
        equipment: ["Kahvakuula"],
        muscles: ["Kädet", "Ojentajat"],
        tags: ["Kädet"],
      },
      {
        name: "Pystysoutu",
      ...
      {
        name: "Työntö selinmakuulta",
        equipment: ["Kahvakuula", "Matto"],
        muscles: ["Kädet"],
        tags: ["Kädet"],
        sideSwitch: true,
      },
      {
        name: "Hauiskääntö + pystypunnerrus",
        equipment: ["Kahvakuula"],
        muscles: ["Kädet", "Hauis", "Hartiat"],
        tags: ["Kädet"],
      },
      {
        name: "Hauiskääntö + ojentajat",
        equipment: ["Kahvakuula"],
        muscles: ["Kädet", "Hauis", "Ojentajat", "Hartiat"],
        tags: ["Kädet"],
      },
      {
        name: "Pystypunnerrus",
      ...
      {
        name: "Pystypunnerrus + ojentajat",
        equipment: ["Kahvakuula"],
        muscles: ["Kädet", "Ojentajat", "Hartiat"],
        tags: ["Kädet"],
      },
      {
        name: "Kulmasoutu",
        equipment: ["Kahvakuula"],
        muscles: ["Kädet", "Hauis", "Kyljet"],
        tags: ["Kädet"],
        sideSwitch: true,
      },
      {
        name: "Kulmasoutu punnerrusasennossa",
        equipment: ["Kahvakuula"],
        muscles: ["Vatsat", "Kädet", "Hauis", "Kyljet"],
        tags: ["Kädet"],
        sideSwitch: true,
      },
      {
      ...
      {
        name: "Pistoolityöntö",
        equipment: ["Kahvakuula (pieni)"],
        muscles: ["Kädet", "Hartiat", "Hauikset"],
        tags: ["Kädet"],
        sideSwitch: true,
      },
      {
        name: "Nyrkkeily kuulalla",
        equipment: ["Kahvakuula"],
        muscles: ["Kädet", "Hartiat", "Kyljet"],
        tags: ["Kädet"],
      },
      {
        name: "Kyljet",
      ...
      // Vatsat / selkä
      {
        name: "Twist",
        equipment: ["Kahvakuula", "Matto"],
        muscles: ["Vatsat", "Kyljet"],
        tags: ["Vatsat"],
      },
      {
        name: "Rutistus kuulan kanssa",
        equipment: ["Kahvakuula", "Matto"],
        muscles: ["Vatsat"],
        tags: ["Vatsat"],
      },
      {
        name: "Kantapäiden kosketus",
        muscles: ["Vatsat", "Kyljet"],
        tags: ["Kehonpaino", "Vatsat"],
      },
      {
        name: "Pull Over",
        equipment: ["Kahvakuula", "Matto"],
      ...
      // Core
      {
        name: "Etuheilautus ja pistoolityöntö lankkuasennosta",
        equipment: ["Kahvakuula"],
        muscles: ["Kädet", "Hartiat", "Kyljet"],
        tags: ["Core", "Sykeliike"],
      },
      {
        name: "Etuheilautus kahdella kädellä",
        equipment: ["Kahvakuula"],
        muscles: ["Vatsat", "Hartiat", "Jalat", "Pakarat"],
        tags: ["Core"],
      },
      {
        name: "Etuheilautus yhdellä kädellä",
        equipment: ["Kahvakuula"],
        muscles: ["Vatsat", "Hartiat", "Jalat", "Pakarat"],
        tags: ["Core"],
        sideSwitch: true,
      ...
      {
        name: "Etuheilautus käden vaihdolla",
        equipment: ["Kahvakuula"],
        muscles: ["Vatsat", "Hartiat", "Jalat", "Pakarat"],
        tags: ["Core"],
      },
      {
        name: "Puolikuu",
        equipment: ["Kahvakuula"],
        muscles: ["Vatsat", "Hartiat", "Jalat"],
        tags: ["Core"],
      },
      {
        name: "Etuheilautus ja kyykky",
        equipment: ["Kahvakuula"],
        muscles: ["Vatsat", "Hartiat", "Jalat", "Pakarat"],
        tags: ["Core", "Sykeliike"],
      },
      {
        name: "Tempaus",
        equipment: ["Kahvakuula"],
        muscles: ["Kädet", "Vatsat", "Hartiat", "Jalat", "Pakarat"],
        tags: ["Core", "Kädet"],
        sideSwitch: true,
      },
      {
        name: "Tempaus kahdella kädellä",
        equipment: ["Kahvakuula"],
        muscles: ["Kädet", "Vatsat", "Hartiat", "Jalat", "Pakarat"],
        tags: ["Core", "Kädet"],
      },
      {
        name: "Rinnalle veto",
        equipment: ["Kahvakuula"],
        muscles: ["Vatsat", "Jalat", "Kädet", "Hartiat"],
        tags: ["Core"],
        sideSwitch: true,
      },
      {
        name: "Kyykky + rinnalle veto",
        equipment: ["Kahvakuula"],
        muscles: ["Vatsat", "Jalat", "Pakarat", "Kädet", "Hartiat"],
        tags: ["Core"],
        sideSwitch: true,
      },
      {
        ...
        equipment: ["Matto"],
        muscles: ["Vatsat"],
        tags: ["Kehonpaino", "Core", "Vatsat"],
        tempo: 0,
      },
      {
        name: "Kylkilankku",
        equipment: ["Matto"],
        muscles: ["Vatsa, Kyljet"],
        tags: ["Kehonpaino", "Core"],
        tempo: 0,
        sideSwitch: true,
      },
      // Sykeliikkeet
      {
        name: "Burbees",
        muscles: ["Kädet", "Vatsat", "Jalat"],
        tags: ["Kehonpaino", "Sykeliike"],
      },
      {
        name: "Burbees + pystysoutu",
        equipment: ["Kahvakuula"],
        muscles: ["Kädet", "Vatsat", "Jalat"],
        tags: ["Sykeliike"],
      },
      {
        name: "Mittarimatoburbees",
        muscles: ["Kädet", "Vatsat", "Jalat"],
        tags: ["Kehonpaino", "Sykeliike"],
      },
      {
        name: "Kyykkyhyppy",
        muscles: ["Pakarat", "Jalat"],
        tags: ["Kehonpaino", "Sykeliike", "Jalat"],
      },
      {
        name: "Heinäsirkka",
        muscles: ["Pakarat", "Jalat"],
      ...
      },
      {
        ...
        equipment: ["Matto"],
        muscles: ["Vatsat"],
        tags: ["Kehonpaino", "Sykeliike", "Vatsat"],
      },
      {
        name: "Kyynärpäille laskeutuminen lankkuasennosta",
        equipment: ["Matto"],
        muscles: ["Vatsat"],
        tags: ["Kehonpaino", "Sykeliike"],
        sideSwitch: true,
      ...
      {
        name: "Turkkilainen nousu",
        equipment: ["Kahvakuula"],
        muscles: ["Jalat", "Kädet", "Vatsat"],
        tempo: 0,
        sideSwitch: true,
      },
    ],
...
app.listen(port, () => {
  ...
  log(
    `SECRET:              ${process.env.SECRET ? "configured" : "undefined"}`
  );
  log(
    `Database configured: ${
  ...
  log(
    `Mail configured:     ${
      ...
      process.env.MAIL_USER_PASS !== undefined
    }`
  );
  log(
    `Feedback email:      ${process.env.MAIL_FEEDBACK_ADDRESS !== undefined}`
  );
});
...
function log(
  ...
  console.log(
    `LOG ${
      ...
      "---------anonymous-user---------"
    } ${requestId} ${new Date().toISOString()} ${message}`
  );
}
